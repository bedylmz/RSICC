...
from mg_models.transformer.attention import MultiHeadAttention
from mg_models.transformer.utils import sinusoid_encoding_table, PositionWiseFeedForward
from mg_models.containers import Module, ModuleList
...
class MeshedDecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):  # decoder(seq, enc_output, mask_enc)
        self_att = self.self_att(input, input, input, mask_self_att, input_gl=None,
                                 isencoder=False)
        self_att = self_att * mask_pad
        enc_att1 = self.enc_att(self_att, enc_output[:, 0], enc_output[:, 0], mask_enc_att) * mask_pad  # 10 15 512
        enc_att2 = self.enc_att(self_att, enc_output[:, 1], enc_output[:, 1], mask_enc_att) * mask_pad  # 10 15 512
        enc_att3 = self.enc_att(self_att, enc_output[:, 2], enc_output[:, 2], mask_enc_att) * mask_pad  # 10 15 512
        alpha1 = torch.sigmoid(self.fc_alpha1(torch.cat([self_att, enc_att1], -1)))  # 10 15 512
        alpha2 = torch.sigmoid(self.fc_alpha2(torch.cat([self_att, enc_att2], -1)))  # 10 15 512
        alpha3 = torch.sigmoid(self.fc_alpha3(torch.cat([self_att, enc_att3], -1)))  # 10 15 512
        enc_att = (enc_att1 * alpha1 + enc_att2 * alpha2 + enc_att3 * alpha3) / np.sqrt(3)
        enc_att = enc_att * mask_pad # 10 15 512
