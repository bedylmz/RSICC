...
def evaluate_transformer(args,encoder_image,encoder_feat,decoder):
    ...
    with open(r'C:\Users\TUBITAK\Desktop\RSICC_v2\SECONDCCpap\datasets\image_hashes.json') as f:
    ...
    with torch.no_grad():
        for i, (image_pairs, caps, caplens, allcaps) in enumerate(
            ...
            el = imgs_A[0]
            converted_el = el.permute(1, 2, 0).contiguous()
            converted_el = converted_el * 255
            converted_el = converted_el.to(torch.int)
            # Flatten the tensor to 1 dimension
            flattened_tensor = converted_el.view(-1).cpu().numpy().astype('uint8')

            # Convert the flattened tensor to a list of integers
            pixel_data = flattened_tensor.tolist()

            # Convert pixel data to bytes
            pixel_bytes = bytes(pixel_data)

            # Compute the SHA-256 hash of the pixel data
            image_hash = hashlib.sha256(pixel_bytes).hexdigest()

            #if image_hash in image_hashes:
            #found')

            #total_results_map["filenames"].append(file_name_png)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default=r"C:\Users\TUBITAK\Desktop\RSICC_v2\SECONDCCpap\createdFileBlackAUG",help='folder with data files saved by create_input_files.py.')
    ...
    parser.add_argument('--path', default=r".\model_sonucları\20241028_RSICCformerSadeceRGB\model_dir", help='model checkpoint.')
    #parser.add_argument('--path', default="C:/Users/TUBITAK/Desktop/RSICC/models_checkpoint/",
     #                   help='model checkpoint.')
    parser.add_argument('--terminal_output' ,default=r".\model_sonucları\20241028_RSICCformerSadeceRGB", help='saving location for terminal output')
    ...
    file_name = f"{args.terminal_output}/test_outputRGB.txt"
