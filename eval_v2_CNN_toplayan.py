...
text_terminal = ""
total_results_map = {}
total_results_map["hypoteses"] = []
total_results_map["references"] = []
total_results_map["filenames"] = []
...
def evaluate_transformer(args, encoder_image, siglip_encoder_image, encoder_feat, decoder):
    global text_terminal

    image_hashes = {}

    with open(r'C:\Users\AliCan\Desktop\datasetsRSICC_v2\image_hashes.json') as f:
        image_hashes = json.load(f)
    image_hashes = {value:key for key,value in image_hashes.items()}
    # Load model
    ...
    with torch.no_grad():
        for i, (image_pairs, caps, caplens, allcaps) in enumerate(
            ...
            siglip_imgs_A = image_pairs[:, 0, :, :, :]
            siglip_imgs_B = image_pairs[:, 1, :, :, :]
            siglip_encoded = []
            ...
            # clip image encoder 
            #for imgA,imgB in zip(siglip_imgs_A,siglip_imgs_B):
            encoded = model_arrange.encode_rgb_images_for_rsformer(siglip_encoder_image,siglip_imgs_A,siglip_imgs_B,device)
                #siglip_encoded.append(encoded.reshape(2,196,768))

            #print(encoded.shape)

            encoded = encoded.reshape(1,2,196,768)

            encoded = encoded.permute(0,1,3,2)
            encoded = encoded.reshape(1,2,768,14,14)

           #siglip_encoded = torch.stack(siglip_encoded)

            encoder_out = encoder_feat(#imgs_A,
                                     #imgs_B,
                                     encoded) # encoder_out: (S, batch, feature_dim)
            ...
            if (len(complete_seqs_scores) > 0):
                ...
                ###Cumlelerden referans ve hipotez jsonini olusturma
    ...
    for ref_part in references:
        ...
        total_results_map["references"].append(img_captions_to_text)
    for hyp_part in hypotheses:
        hyp_text = ""
        for el in hyp_part:
            hyp_text = hyp_text + rev_word_map[el] + " "
        total_results_map["hypoteses"].append(hyp_text)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default=r"C:\Users\AliCanKaraca\Desktop\DL\input_with_SM_farkli_modeller_6041_2NC_fixed",help='folder with data files saved by create_input_files.py.')
    ...
    parser.add_argument('--path', default=r"C:\Users\TUBITAK\Desktop\RSICC_v2\farkli_modeller\CNN_toplayan\model_sonucları\20240716_6041_2NC_toplayan_1\model_dir", help='model checkpoint.')
    #parser.add_argument('--path', default="C:/Users/TUBITAK/Desktop/RSICC/models_checkpoint/",
     #                   help='model checkpoint.')
    parser.add_argument('--terminal_output' ,default=r"C:\Users\TUBITAK\Desktop\RSICC_v2\farkli_modeller\CNN_toplayan\model_sonucları\20240716_6041_2NC_toplayan_1", help='saving location for terminal output')
    ...
    for i in range(len(filename)):
        ...
        if args.decoder == "trans":
            ...
            metrics,_,_ = evaluate_transformer(args,encoder_image,encoder_feat,decoder)

        print_with_json("beam size {}: BLEU-1 {} BLEU-2 {} BLEU-3 {} BLEU-4 {} ROUGE_L {} CIDEr {} SPICE {} METEOR {}".format
                        (args.beam_size, metrics["Bleu_1"], metrics["Bleu_2"], metrics["Bleu_3"],
                         metrics["Bleu_4"],
                         metrics["ROUGE_L"], metrics["CIDEr"], metrics["SPICE"], metrics["METEOR"] ))
    ...
    file_name = f"{args.terminal_output}/test_output.txt"
    with open(file_name, 'w') as dosya:
        dosya.write(text_terminal)
    with open(f"{args.terminal_output}/references_and_hypoteses.json", "w") as f:
        json.dump(total_results_map, f)
