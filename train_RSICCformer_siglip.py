...
def train(
    ...
    for i, (img_pairs, caps, caplens) in enumerate(train_loader):
        ...
        siglip_imgs_A = img_pairs[:, 0, :, :, :]
        siglip_imgs_B = img_pairs[:, 1, :, :, :]
        siglip_encoded = []
        ...
        encoded = model_arrange.encode_rgb_images_for_rsformer(siglip_encoder_image,siglip_imgs_A,siglip_imgs_B,device)
            #encoded = model_arrange.encode_rgb_images_for_rsformer(siglip_encoder_image,imgA,imgB,device)

        #print("end"*3)
        #print(encoded.shape)

        encoded = encoded.reshape(encoded.shape[0],2,196,768)

        encoded = encoded.permute(0,1,3,2)
        encoded = encoded.reshape(encoded.shape[0],2,768,14,14)
...
def main(args, meteor_output=None):
    ...
    siglip_encoder_image = model_arrange.load_model("C:/Users/AliCan/Desktop/clip4idc/ckpts/siglip/pytorch_model.bin.31")
    # encoder_image2 = CNN_Encoder(NetType=args.encoder_image, method=args.decoder)

    siglip_encoder_image = siglip_encoder_image.to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train(
            ...
            siglip_encoder_image=siglip_encoder_image,
        ...
        metrics, nochange_metrics, change_metrics = evaluate_transformer(
            args, encoder_image=encoder_image,siglip_encoder_image=siglip_encoder_image, encoder_feat=encoder_feat, decoder=decoder
        )
