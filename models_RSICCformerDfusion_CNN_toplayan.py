...
class FeatureFusionModule(nn.Module):
    def __init__(self, feature_dim):
        ...
        # Use Conv2d for spatial data instead of Linear
        self.fusion_layer = nn.Conv2d(feature_dim * 2, feature_dim, kernel_size=1)
        self.activation = nn.ReLU()
        
    def forward(self, visual_features, semantic_features):
        # This works with [26, 512, 7, 7] tensors
        concatenated_features = torch.cat([visual_features, semantic_features], dim=1)
        # Now concatenated_features is [26, 1024, 7, 7]
        fused_features = self.fusion_layer(concatenated_features)
        # fused_features becomes [26, 512, 7, 7] again
...
class MCCFormers_diff_as_Q(nn.Module):
    ...
    def __init__(self, feature_dim, dropout, h, w, d_model=512, n_head=4, n_layers=3):
        ...
        self.duality_model_upsampler = DualModalityUpsampler()
        ...
        self.projection5 = nn.Conv2d(2048, d_model, kernel_size=1)
        self.projection6 = nn.Conv2d(4096, d_model, kernel_size=1)
        self.projection7 = nn.Conv2d(1536, d_model, kernel_size=1)
    ...
    def forward(self,
        ...
        #img_feat1 = img_feat1.float() # img_feat1 torch.Size([26, 1024, 14, 14])
        #img_feat2 = img_feat2.float() # img_feat2 torch.Size([26, 1024, 14, 14])
        #clip_encoded = clip_encoded.float() # clip_encoded torch.Size([26, 2, 7, 7, 768])

        
        #print("*"*50)
        #print("img_feat1",img_feat1.shape)
        #print("img_feat2",img_feat2.shape)
        #print("clip_encoded",clip_encoded.shape)
        #print("*"*50)

        #before, after = self.duality_model_upsampler(clip_encoded)
 
        #* img_feat1 = before # + img_feat1
        #* img_feat2 = after # + img_feat2


        #print(type(clip_encoded))
        ...
        #print(img_feat1[0].size())

        #print(img_feat2.size())
        #print(img_feat2[0].size())
        #print("----"*20)
        ...
        if feature_dim == 1024:
            ...
            # sem_feat1 = self.projection(sem_feat1)
            # sem_feat2 = self.projection(sem_feat2)
        ...
        if feature_dim == 512:
            ...
            # sem_feat1 = self.projection3(sem_feat1)
            # sem_feat2 = self.projection3(sem_feat2)
        if feature_dim == 256:
            img_feat1 = self.projection4(img_feat1)  
            img_feat2 = self.projection4(img_feat2)
            #print(" 256 "*50)

            # sem_feat1 = self.projection4(sem_feat1)
            # sem_feat2 = self.projection4(sem_feat2)
        if feature_dim == 2048:
            img_feat1 = self.projection5(img_feat1)
            img_feat2 = self.projection5(img_feat2)
            #print(" 2048 "*50)

            # sem_feat1 = self.projection5(sem_feat1)
            # sem_feat2 = self.projection5(sem_feat2)
        ...
        # sem_feat1 = sem_feat1 + position_embedding
        # sem_feat2 = sem_feat2 + position_embedding
        ...
        # encoder_output3 = sem_feat1.view(batch, self.d_model, -1).permute(2, 0, 1)  # (h*w, batch_size, d_model)
        # encoder_output4 = sem_feat2.view(batch, self.d_model, -1).permute(2, 0, 1)  # (h*w, batch_size, d_model)

        #output1 = encoder_output1 #+ encoder_output3
        #output2 = encoder_output2 #+ encoder_output4
        output1_list = list()
        output2_list = list()
        for l in self.transformer:
            output1, output2 = l(encoder_output1, encoder_output2)
        ...
        for res in self.resblock:
            input = torch.cat([output1_list[i],output2_list[i]],dim=-1) # -> torch.Size([49, 26, 1024])
            output = output + input
...
class DualModalityUpsampler(nn.Module):
    def __init__(self, in_channels=768, out_channels=768, dtype=torch.float32):
        super(DualModalityUpsampler, self).__init__()
        self.dtype = dtype

        # Ortak bir modül tanımlayıp iki kez kullanacağız
        self.path1 = nn.Sequential(
            nn.Conv2d(in_channels, 512, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(512, out_channels, kernel_size=4, stride=2, padding=1),  # 7x7 -> 14x14
            nn.ReLU(inplace=True)
        ).to(dtype=self.dtype)
    ...
    def forward(self, clip_encoded):
        """
        clip_encoded: Tensor of shape [B, 2, 7, 7, 768]
        returns: out1, out2: [B, 768, 14, 14] in self.dtype
        """

        # Modaliteleri ayır
        mod1 = clip_encoded[:, 0].permute(0, 3, 1, 2).to(self.dtype)  # [B, 768, 7, 7]
        mod2 = clip_encoded[:, 1].permute(0, 3, 1, 2).to(self.dtype)  # [B, 768, 7, 7]

        # Aynı ağı iki kez kullan
        out1 = self.path1(mod1)  # [B, 768, 14, 14]
        out2 = self.path2(mod2)  # [B, 768, 14, 14]
